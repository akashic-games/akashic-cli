	...
	render(): React.ReactNode {
		...
		return <div className={styles["dialog"]}>
			...
			<div className={styles["button-column"]}>
				<button id="acceptToDisplayPlayerInfo" className={styles["button"]} onClick={() => onClick(true)}>{name} (プレイヤー名)</button>
				<button id="rejectToDisplayPlayerInfo" className={styles["button"]} onClick={() => onClick(false)}>{guestName}</button>
			</div>
		</div>;
	}
}
