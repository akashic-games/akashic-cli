...
export class Operator {
	...
	startContent = async (params?: StartContentParameterObject): Promise<void> => {
		...
		const tokenResult = await apiClientLocalHost.createPlayToken(play.playId, store.player.id, false, store.player.name);
	...
	restartWithNewPlay = async (): Promise<void> => {
		...
		await apiClientLocalHost.broadcast(this.store.currentPlay.playId, { type: "switchPlay", nextPlayId: play.playId });
	...
	private async _createServerLoop(contentLocator: ClientContentLocator, audioState?: PlayAudioState): Promise<PlayEntity> {
		...
		const tokenResult = await apiClientLocalHost.createPlayToken(play.playId, "", true);  // TODO 空文字列でなくnullを使う
		await play.createServerInstance({ playToken: tokenResult.data.playToken });
		await apiClientLocalHost.resumePlayDuration(play.playId);
