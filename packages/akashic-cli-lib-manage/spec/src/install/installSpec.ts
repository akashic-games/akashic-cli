...
describe("install()", function () {
	...
	it("installs modules and update globalScripts", function (done) {
		...
		Promise.resolve()
			...
			.then(() => promiseInstall({ moduleNames: ["noOmitPackagejson@0.0.0"], cwd: "./somedir", logger: logger, debugNpm: dummyNpm, noOmitPackagejson: true, tmpMmp: true }))
			.then(() => cmn.ConfigurationFile.read("./somedir/game.json", logger))
			.then((content) => {
				const globalScripts = content.globalScripts;
			...
			.then(() => promiseInstall({ moduleNames: ["dummy@1.0.1"], cwd: "./somedir", logger: logger, debugNpm: dummyNpm, tmpMmp: true }))
			.then(() => cmn.ConfigurationFile.read("./somedir/game.json", logger))
			.then((content) => {
				const globalScripts = content.globalScripts;
