...
describe("exportHTML", function () {
	it("_completeExportHTMLParameterObject", function (done: jest.DoneCallback) {
		Promise.resolve()
			.then(function () {
	...
	it("_completeExportHTMLParameterObject - fullpath", function (done: jest.DoneCallback) {
		Promise.resolve()
			.then(function () {
	...
	it("promiseExportHTML", function (done: jest.DoneCallback) {
		Promise.resolve()
			.then(function () {
				const param: exp.ExportHTMLParameterObject = {
					logger: undefined,
	...
	it("output option has zip extension", function (done: jest.DoneCallback) {
		Promise.resolve()
			.then(function () {
				const param: exp.ExportHTMLParameterObject = {
					logger: undefined,
