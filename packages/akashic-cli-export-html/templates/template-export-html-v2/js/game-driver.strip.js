    ...
    8: [ function(require, module, exports) {
        ...
        var g = require("@akashic/akashic-engine"), constants = require("./constants"), LoopMode_1 = require("./LoopMode"), LoopRenderMode_1 = require("./LoopRenderMode"), ExecutionMode_1 = require("./ExecutionMode"), Clock_1 = require("./Clock"), ProfilerClock_1 = require("./ProfilerClock"), EventConverter_1 = require("./EventConverter"), TickController_1 = require("./TickController"), GameLoop = function() {
            function GameLoop(param) {
                ...
                this._omittedTickDuration = 0, param.errorHandler && this.errorTrigger.add(param.errorHandler, param.errorHandlerOwner);
                ...
                this._setLoopRenderMode(loopRenderMode), this._game.setIsSkipAware(null != conf.skipAwareGame ? conf.skipAwareGame : !0), 
            ...
            return GameLoop.prototype.start = function() {
                ...
                this._sceneTickMode = tickMode, this._clock.frameTrigger.remove(this._onFrame, this), 
                this._clock.frameTrigger.remove(this._onLocalFrame, this), localMode) {
                  case g.LocalTickMode.FullLocal:
                    this._tickController.stopTick(), this._clock.frameTrigger.add(this._onLocalFrame, this);
                    break;
                ...
                this._omittedTickDuration = 0, sceneChanged && this._handleSceneChange();
                ...
                if (this._loopMode === LoopMode_1["default"].Replay && this._targetTimeFunc) {
                    ...
                    this._onFrameForTimedReplay(targetTime, frameArg), prevTime === this._currentTime && targetTime >= prevTime && targetTime <= prevTime + this._frameTime && this.rawTargetTimeReachedTrigger.fire(givenTargetTime);
                ...
                }, this._onGotStartPoint_bound)), 0 >= frameGap) return void (this._skipping && this._stopSkipping());
                ...
                for (var consumedFrame = 0; consumedFrame < this._skipTicksAtOnce; ++consumedFrame) {
                    ...
                    if (null == nextTickTime && (nextTickTime = nextFrameTime), nextFrameTime > targetTime) {
                        ...
                        if (nextTickTime >= targetTime) {
                            this._omittedTickDuration += targetTime - this._currentTime, this._currentTime = Math.floor(targetTime / this._frameTime) * this._frameTime;
                            break;
                        }
                        nextFrameTime = nextTickTime, this._omittedTickDuration += nextTickTime - this._currentTime;
                    }
                    ...
                    if ("number" == typeof tick) consumedAge = tick, sceneChanged = game.tick(!0, Math.floor(this._omittedTickDuration / this._frameTime)); else {
                        consumedAge = tick[0];
                        var pevs_1 = tick[1];
                    ...
                    if (this._omittedTickDuration = 0, game._notifyPassedAgeTable[consumedAge] && game.fireAgePassedIfNeeded()) {
                ...
                ageGap = 1) : (targetAge = this._targetAge, ageGap = targetAge - currentAge), (ageGap > this._jumpTryThreshold || 0 > ageGap) && !this._waitingStartPoint && this._lastRequestedStartPointAge < currentAge && (this._waitingStartPoint = !0, 
                this._lastRequestedStartPointAge = targetAge, this._amflow.getStartPoint({
                    frame: targetAge
                }, this._onGotStartPoint_bound)), 0 >= ageGap) return 0 === ageGap && (0 === currentAge && this._tickBuffer.requestTicks(), 
                ...
                for (var loopCount = !this._skipping && ageGap <= this._delayIgnoreThreshold ? 1 : Math.min(ageGap, this._skipTicksAtOnce), consumedFrame = 0; loopCount > consumedFrame; ++consumedFrame) {
                    var nextFrameTime = this._currentTime + this._frameTime, nextTickTime = this._tickBuffer.readNextTickTime();
                    if (null != nextTickTime && nextTickTime > nextFrameTime) {
                        ...
                        nextFrameTime = Math.ceil(nextTickTime / this._frameTime) * this._frameTime, this._omittedTickDuration += nextFrameTime - this._currentTime;
                    }
                    this._currentTime = nextFrameTime;
                    ...
                    if (pevs) for (var i = 0, len = pevs.length; len > i; ++i) game.events.push(this._eventConverter.toGameEvent(pevs[i]));
                    if ("number" == typeof tick) consumedAge = tick, sceneChanged = game.tick(!0, Math.floor(this._omittedTickDuration / this._frameTime)); else {
                        consumedAge = tick[0];
                        var pevs_2 = tick[1];
                ...
                this._clock.frameTrigger.remove(this._onEventsProcessed, this), this._skipping && this._stopSkipping(), 
                ...
                this._omittedTickDuration = 0, this._game._restartWithSnapshot(startPoint), this._handleSceneChange();
